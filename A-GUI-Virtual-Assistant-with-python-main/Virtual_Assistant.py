...
def Process_audio():
    ...
    if __name__=='__main__':
        while run==1:
            ...
            app_link = [r'\Microsoft Office Word 2007.lnk',r'\PowerPoint.lnk', r'\Microsoft Office Excel 2007.lnk', r'\Zoom.lnk', r'\Notepad.lnk', r'\Google Chrome.lnk' ]
            ...
            if "hello" in statement.split() or "hi" in statement.split():

              wishMe()               

            if  "goodbye" in statement or "good bye" in statement or "ok bye" in statement or "stop" in statement:
                speak('Your personal assistant ' + name_assistant +' is shutting down, bye bye')
                screen.destroy()
                break
            ...
            elif "weather in" in statement.lower():
                location = statement[11:]  # Get the location from the command
                weather_api_key = "b583fcae4c0f15793831ab7ed72e0c3a"  # Replace with your OpenWeatherMap API key
                weather_url = f"http://api.openweathermap.org/data/2.5/weather?q={location}&appid={weather_api_key}"
                response = requests.get(weather_url)
                if response.status_code == 200:
                    weather_data = response.json()
                    temp = round(weather_data["main"]["temp"] - 273.15)  # Convert from Kelvin to Celsius
                    description = weather_data["weather"][0]["description"]
                    speak(f"The temperature in {location} is {temp} degrees Celsius and the weather is {description}.")
                else:
                    speak("Sorry, I could not get the weather for that location.")

            elif "search for" in statement.lower():
                query = statement[11:]  # Get the search query from the command
            ...
            if "who made you" in statement or "who created you" in statement or "who discovered you" in statement:
                speak("I was built by Aparna and kumkum")
...
def info():
  ...
  creator_label = Label(info_screen,text = "Created by Aparna Jaiswal and Kumkum Bagha")
  creator_label.pack()

  Age_label = Label(info_screen, text= "Rng.No.:360 and 367")
  Age_label.pack()

  for_label = Label(info_screen, text = "For SEPM Project")
  for_label.pack()
...
def main_screen():
      ...
      screen.config(bg="#a4bddc")
    


      name_label = Label(text = name_assistant,width = 300, bg = "white", fg="#7d91a9", font = ("Poppins", 40))
      name_label.pack()
